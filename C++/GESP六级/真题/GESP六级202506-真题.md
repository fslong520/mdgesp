...
## 第 7 题【完全二叉树数组存储】(2分)
在使用数组表示完全二叉树时，如果一个节点的索引为 i（从 0 开始计数），那么其左孩子节点的索引通常是（ ）。
- A. `(i - 1) / 2`
- B. `i + 1`
- C. `i * 2`
- D. `2 * i + 1`
...
## 第 11 题【DFS填空】(2分)
请将下列 C++ 实现的深度优先搜索（DFS）代码补充完整，横线处应填（ ）。
```cpp
struct TreeNode {
    int val;
    TreeNode* left;
...
## 第 12 题【BFS每层最大】(2分)
给定一棵二叉树，返回每层中最大的节点值，结果以数组形式返回，横线处应填（ ）。
```cpp
#include <vector>
#include <queue>
#include <algorithm>
...
## 第 15 题【0/1背包一维】(2分)
给定 n 个物品和一个最大承重为 W 的背包，每个物品有一个重量和价值，每个物品只能选择放或不放。目标是选择若干个物品放入背包，使得总价值最大，且总重量不超过 W。关于下面代码，说法正确的是（ ）。
```cpp
...
## 第 3 题
为了实现一个队列，使其出队操作（pop）的时间复杂度为 O(1)，并且避免数组删除首元素的问题，一种常见且有效的方法是使用环形数组，通过调整队首和队尾指针来实现。
（　）
...
## 第 8 题
下列代码中的 tree 向量，表示的是棵完全二叉树（-1 代表空节点）按照层序遍历的结果。
```cpp
#include <vector>
...
## 第 26 题 <a href="https://fslong.iok.la/problem/GESP202506-L6-1" target="_blank">26、学习小组（25 分）</a>

**题目描述**

班主任计划将班级里的 n 名同学划分为若干个学习小组，每名同学都需要分入某一个学习小组中。观察发现，如果一个学习小组中恰好包含 i 名同学，则该学习小组的讨论积极度为 a[i]。

给定讨论积极度 a[i]，请你计算将这 n 名同学划分为学习小组的所有可能方案中，讨论积极度之和的最大值。

**输入格式**

第一行，一个正整数 n，表示班级人数。

第二行，n 个非负整数 a[1], a[2], ..., a[n]，表示不同人数学习小组的讨论积极度。
...
## 第 27 题 <a href="https://fslong.iok.la/problem/GESP202506-L6-2" target="_blank">27、最大因数（25 分）</a>

**题目描述**

给定一棵有 n 个结点的有根树，这些结点依次以 1, 2, ..., n 编号，根结点的编号为 1。对于编号为 i（1 < i ≤ n）的结点，其父结点的编号为 i 的因数中除 i 以外最大的因数。

现在有 q 组询问，第 j（1 ≤ j ≤ q）组询问给定 u, v，请你求出编号分别为 u 和 v 的两个结点在这棵树上的距离。两个结点之间的距离是连接这两个结点的简单路径所包含的边数。

**输入格式**

第一行，一个正整数 q，表示询问组数。

接下来 q 行，每行两个正整数 u, v，表示询问结点的编号。

**输出格式**

输出共 q 行，每行一个整数，表示结点 u, v 之间的距离。
...
对于所有测试点，保证 1 ≤ n ≤ 10⁹，1 ≤ q ≤ 10³，1 ≤ u, v ≤ n。
