---
title: "【GESP】C++等级考试5级 2025年06月 真题"
...
- A. gcd0() 函数的时间复杂度为 O(log(min(a,b)))
- B. gcd1() 函数的时间复杂度为 O(min(a,b))
...
### 第 4 题【归并排序输出次数】(2分)
下面的C++代码实现归并排序。代码在执行时，将输出 1 次 HERE 字符串，因为merge()函数仅被调用 1 次。
```cpp
...
### 第 6 题【二分查找应用】(2分)
查字典这个小学生必备技能，可以把字典视为一个已排序的数组。假设小杨要查找一个首字母为 g 的单词，他首先翻到字典约一半的页数，发现该页的首字母是 m，由于字母表中 g 位于 m 之前，所以排除字典后半部分，查找范围缩小到前半部分；不断重复上述步骤，直至找到首字母为 g 的页码。这种查字典的一系列操作可看作二分查找。
...
### 编程题 1【[奖品兑换](https://fslong.iok.la/problem/GESP202506-5-T1)】(25分)
班主任给上课专心听讲、认真完成作业的同学们分别发放了若干张课堂优秀券和作业优秀券。同学们可以使用这两种券找班主任兑换奖品。具体来说，可以使用 a 张课堂优秀券和 b 张作业优秀券兑换一份奖品，或者使用 c 张课堂优秀券和 d 张作业优秀券兑换一份奖品。
...
**数据范围：**
对于 30% 的测试点，保证 n, m ≤ 1000，a, b ≤ 100。
对于所有测试点，保证 1 ≤ n, m ≤ 10^9，1 ≤ a, b ≤ 10^9。
...
### 编程题 2【[最大公因数](https://fslong.iok.la/problem/GESP202506-5-T2)】(25分)
对于两个正整数 a, b，它们的最大公因数记为 gcd(a, b)。对于 n 个正整数 a_1, a_2, ..., a_n，它们的最大公因数为：gcd(a_1, a_2, ..., a_n) = gcd(gcd(a_1, a_2), a_3, ..., a_n)。

给定 n 个正整数 a_1, a_2, ..., a_n 以及 q 组询问。对于第 i（1 ≤ i ≤ q）组询问，请求出 gcd(a_1+i, a_2+i, ..., a_n+i) 的最大公因数，也即 gcd(a_1+i, a_2+i, ..., a_n+i)。
...
**输出格式：**
输出共 q 行，第 i 行包含 1 个正整数，表示 gcd(a_1+i, a_2+i, ..., a_n+i) 的最大公因数。
...
**数据范围：**
对于 30% 的测试点，保证 n, q ≤ 1000，a_i ≤ 10^6。
对于所有测试点，保证 1 ≤ n, q ≤ 10^5，1 ≤ a_i ≤ 10^9。
