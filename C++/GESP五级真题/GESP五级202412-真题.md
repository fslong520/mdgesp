---
title: "【GESP】C++等级考试5级 2024年12月 真题"
...
- B. fibA采用递推方式。
- C. fibB采用的是递归方式。
- D. fibA时间复杂度为 O(n)，fibB的时间复杂度为 O(2^n)。

---

### 第 5 题【辗转相除法调用顺序】(2分)
两块长方形土地的长宽分别为 24米 和 36米，要将它们分成正方形的小块，使得正方形的尺寸尽可能大。小杨采用如下的辗转相除函数 gcd(24, 36) 来求正方形分块的边长，则函数 gcd 调用顺序为（ ）。
```cpp
int gcd(int a, int b) {
    int big = a > b ? a : b;
    int small = a < b ? a : b;
...
### 第 7 题【埃氏筛法】(2分)
下述代码实现素数表的埃拉托斯特尼(埃氏)筛法，筛选出所有小于等于 n 的素数。
```cpp
...
- A. 快速排序通过递归对子问题进行求解。
- B. 快速排序的最坏时间复杂度是 O(n^2)。
- C. 快速排序是一个稳定的排序算法。
- D. 在最优情况下，快速排序的时间复杂度是 O(n log n)。
...
### 编程题 1【[奇妙数字](https://fslong.iok.la/problem/FSL4650)】(25分)
小杨认为一个数字 x 是奇妙数字当且仅当 x = p^k，其中 p 为任意质数且 k 为正整数。例如，8 = 2^3，所以 8 是奇妙数字，6 不是。

对于一个正整数 N，小杨想要构建一个包含若干个奇妙数字的集合 S，使其满足以下条件：
1. 集合中不包含相同的数字。
2. 集合中所有数字的乘积是 N 的因数。
...
**样例解释：**
关于本样例，符合题意的一个包含 3 个奇妙数字的集合是 {2, 4, 8}。首先，因为 2=2^1，4=2^2，8=2^3，所以 2, 4, 8 均为奇妙数字。同时，2×4×8=64 是 128 的因数。
由于无法找到符合题意且同时包含 4 个奇妙数字的集合，因此本样例的答案为 3。

**数据范围：**
对于全部数据，保证有 1 ≤ N ≤ 10^12。
...
### 编程题 2【[武器强化](https://fslong.iok.la/problem/FSL4651)】(25分)
...
**数据范围：**
对于全部数据，保证有 1 ≤ N, M ≤ 1000，1 ≤ P_i ≤ N，1 ≤ C_i ≤ 10^6。
