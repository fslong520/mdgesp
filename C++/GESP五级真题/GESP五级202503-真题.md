---
title: "【GESP】C++等级考试5级 2025年03月 真题"
...
### 第 10 题【归并排序分治】(2分)
归并排序算法体现了分治算法，每次将大的待排序数组分成大小大致相等的两个子数组，然后分别对两个子数组进行排序，最后对排好序的两个子数组合并成有序数组。
...
### 编程题 1【[平均分配](https://fslong.iok.la/problem/P7033)】(25分)
A有 n 件物品，B和C想从A手上买走这些物品。对于第 i 件物品，B会以 b_i 的价格购买，C会以 c_i 的价格购买。为了平均分配这件物品，A决定B和C各只能买恰好 n 件物品。你能帮A求出他卖出这件物品所能获得的最大收入吗？
...
**数据范围：**
对于 30% 的测试点，保证 n ≤ 10。
对于另外 40% 的测试点，保证 n ≤ 1000，b_i = c_i。
对于所有测试点，保证 1 ≤ n ≤ 10^5，0 ≤ b_i, c_i ≤ 10^9。
...
### 编程题 2【[原根](https://fslong.iok.la/problem/P7026)】(25分)
小 A 知道，对于质数 p 而言，g 的原根是满足以下条件的正整数：
1. 1 ≤ g < p；
2. g^(p-1) ≡ 1 (mod p)；
3. 对于任意 1 ≤ k < p-1，均有 g^k ≢ 1 (mod p)。

其中 a ≡ b (mod p) 表示 a 除以 p 的余数等于 b 除以 p 的余数。

小 A 现在有一个整数 g，请你帮他判断 g 是不是 p 的原根。

**输入格式：**
第一行，一个正整数 T，表示测试数据组数。
每组测试数据包含一行，两个正整数 g, p。

**输出格式：**
对于每组测试数据，输出一行，如果 g 是 p 的原根则输出 Yes，否则输出 No。
...
**数据范围：**
对于 30% 的测试点，保证 p ≤ 1000。
对于所有测试点，保证 1 ≤ T ≤ 100，2 ≤ g < p ≤ 10^9，p 为质数。
