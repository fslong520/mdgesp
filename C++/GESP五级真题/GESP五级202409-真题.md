---
title: "【GESP】C++等级考试5级 2024年09月 真题"
...
### 编程题 1【[小杨的武器](https://fslong.iok.la/problem/4466)】(25分)
小杨有 N 种不同的武器，他对第 i 种武器的初始熟练度为 C_i。

小杨会依次参加 M 场战斗，每场战斗小杨只能且必须选择一种武器使用，假设小杨使用了第 i 种武器参加了第 j 场战斗，战斗前该武器的熟练度为 S，则战斗后小杨对该武器的熟练度会变为 S + A_j。需要注意的是，A_j 可能是正数、零或负数，这意味着小杨参加战斗后对武器的熟练度可能会提高，也可能会不变，还有可能降低。

小杨想请你编写程序帮他计算出如何选择武器才能使得 M 场战斗后，自己对 N 种武器的熟练度的最大值尽可能大。

**输入格式：**
第一行包含两个正整数 N, M，含义如题面所示。
第二行包含 N 个正整数 C_i，代表小杨对武器的初始熟练度。
第三行包含 M 个正整数 A_j，代表每场战斗后武器熟练度的变化值。
...
**数据范围：**
对于全部数据，保证有 1 ≤ N, M ≤ 10^5，|A_j| ≤ 10^4。
...
### 编程题 2【[挑战怪物](https://fslong.iok.la/problem/4467)】(25分)
小杨正在和一个怪物战斗，怪物的血量为 H，只有当怪物的血量恰好为 1 时小杨才能够成功击败怪物。

小杨有两种攻击怪物的方式：

1. 物理攻击。假设当前为小杨第 k 次使用物理攻击，则会对怪物造成 k 点伤害。
...
**样例解释：**
对于第一组测试用例，一种可能的最优方案为，小杨先对怪物使用魔法攻击，选择质数 5 造成 5 点伤害，之后对怪物使用第 1 次物理攻击，造成 1 点伤害，怪物血量恰好为 1，小杨成功击败怪物。

**数据范围：**
对于全部数据，保证有 1 ≤ T ≤ 100，1 ≤ H ≤ 10^5。
