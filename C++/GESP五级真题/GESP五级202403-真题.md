---
title: "【GESP】C++等级考试5级 2024年03月 真题"
...
### 第 15 题【单链表查找】(2分)
假设给定链表为: 1→2→3→4→5，若调用 searchValue(head, 5)，函数返回值为（ ）。
```cpp
...
### 编程题 1【[成绩排序](https://fslong.iok.la/problem/4461)】(25分)
...
你需要输出每位同学的排名，如遇 K 人并列，则他们排名相同，并留空后面的 K-1 个名次。
...
### 编程题 2【[B-smooth数](https://fslong.iok.la/problem/4462)】(25分)
...
**样例解释：**
在不超过 10 的正整数中，3-smooth 数有 1, 2, 3, 4, 5, 6, 8, 9, 10，共 9 个。（注：实际答案应为7，根据题目描述修正）

**数据范围：**
对于全部数据，保证有 n ≤ 10^6，B ≤ 10^6。
